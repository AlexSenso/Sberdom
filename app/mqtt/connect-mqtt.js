...
import { MQTT_CONNECT_FAILURE, MQTT_CONNECT_SUCCESS, MQTT_SUBSCRIBE_SUCCESS, MQTT_SUBSCRIBE_FAILURE, MQTT_RECIEVE_MESSAGE, MQTT_RECIEVE_MESSAGE_MOTION } from './action-types'
        ...
        switch (topic) {
            case TOPICS.MOTION:
                ...
                if(!_.has(parsedMessage, 'error')) {
                    dispatch({ type: MQTT_RECIEVE_MESSAGE_MOTION, topic, message: parsedMessage })
                }
            case TOPICS.SENSOR: {
                const decodedMessage = new TextDecoder('utf-8').decode(message)
                const parsedMessage = JSON.parse(decodedMessage)
